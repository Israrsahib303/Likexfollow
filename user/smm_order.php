...
// --- 2. DYNAMIC FILTER RULES (FIXED REFILL BUG) ---
$filter_rules = [
    ...
    'refill'       => 'refill, r30, r60, r90, r365, lifetime', 
...
try {
    ...
    foreach ($all_services as $s) {
        ...
        if ($app_name) {
            ...
            // 🔥 CORRECTED LXF ID GENERATION FOR FRONTEND
            $lxf_id = 'LXF-' . str_pad($s['id'], 4, '0', STR_PAD_LEFT); 

            $services_json[$s['id']] = [
                'id'      => $s['id'],
                'lxf_id'  => $lxf_id, // New Corrected Special ID
...
<head>
    ...
    :root {
        ...
        --ios-green: #34C759;
        --ios-red: #FF3B30;
        --lx-primary: #6366f1; /* Special Theme Purple */
        --lx-primary-light: rgba(99, 102, 241, 0.15);
    ...
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; outline: none; }
    ...
    /* --- 🚀 ULTRA LIVE ANIMATED SEARCH BOX (WITH PASTE BUTTON) --- */
    .search-box-wrapper {
        position: relative;
        margin-bottom: 35px;
        width: 100%;
    ...
    .search-box-wrapper.active-search {
        transform: scale(1.03);
    }
    .search-box-wrapper.active-search::before {
        filter: blur(8px);
        inset: -4px;
        opacity: 1;
    }
    ...
    .search-box-wrapper input {
        width: 100%; 
        padding: 18px 85px 18px 56px; /* Right padding increased for PASTE button */
        background: #ffffff;
        border: none; 
        border-radius: 20px;
        font-size: 16px; 
    ...
    /* Live Text Color Animation on Focus */
    .search-box-wrapper input:focus {
        animation: textGlow 3s infinite;
    }
    @keyframes textGlow {
        0% { color: #000; }
        50% { color: var(--lx-primary); }
        100% { color: #000; }
    }

    /* Live Animated Search Icon */
    .search-icon { 
        position: absolute; left: 22px; top: 50%; transform: translateY(-50%); 
        color: var(--lx-primary); font-size: 18px; 
        animation: iconPulse 2s infinite;
    }
    @keyframes iconPulse {
        0% { transform: translateY(-50%) scale(1) rotate(0deg); opacity: 1; }
        50% { transform: translateY(-50%) scale(1.2) rotate(15deg); opacity: 0.7; color: #ec4899; }
    ...
    .search-paste-btn {
        ...
        background: rgba(99, 102, 241, 0.1); color: var(--lx-primary);
        font-size: 11px; font-weight: 800; padding: 10px 14px;
        border-radius: 12px; border: none; cursor: pointer; transition: 0.2s;
        z-index: 10;
    }
    .search-paste-btn:hover {
        background: var(--lx-primary); color: white;
    }
    .search-paste-btn:active {
        transform: translateY(-50%) scale(0.95);
    }

    /* --- CLICK TO COPY LXF BADGE --- */
    .lxf-badge {
        background: linear-gradient(135deg, var(--lx-primary), #818cf8);
        color: white; padding: 6px 14px; border-radius: 12px; font-size: 12px;
        font-weight: 800; font-family: 'JetBrains Mono', monospace; letter-spacing: 0.5px;
    ...
    h3 { font-size: 22px; font-weight: 700; margin: 0 0 16px 0; color: var(--ios-text); display:flex; align-items:center; }
    ...
    .app-card {
        ...
        cursor: pointer; transition: transform 0.2s cubic-bezier(0.34, 1.56, 0.64, 1); padding: 5px;
    }
    .app-card:hover { transform: translateY(-3px); }
    .app-icon-wrap {
        width: 64px; height: 64px; border-radius: 16px; background: var(--ios-card);
    ...
    /* --- NAVIGATION HEADER --- */
    .nav-header { display: flex; align-items: center; gap: 12px; margin-bottom: 20px; }
    .back-btn {
        width: 36px; height: 36px; border-radius: 50%; background: rgba(118, 118, 128, 0.12); border: none; color: var(--lx-primary); 
        font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; 
        backdrop-filter: blur(5px); flex-shrink: 0;
    }
    .nav-app-icon { width: 32px; height: 32px; border-radius: 8px; object-fit: cover; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
    .nav-title { font-size: 20px; font-weight: 700; color: var(--ios-text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

    /* --- SUB-CATEGORY --- */
    .big-white-box { display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 16px; padding-bottom: 20px; }
    .ios-img-btn { width: 100%; border-radius: var(--radius-m); cursor: pointer; box-shadow: var(--shadow-sm); transition: transform 0.2s, box-shadow 0.2s; background: #fff; }

    /* --- FILTERS --- */
    .filter-wrapper { display: flex; gap: 12px; overflow-x: auto; padding: 4px 4px 20px 4px; -webkit-overflow-scrolling: touch; align-items: center; scrollbar-width: none; }
    .filter-wrapper::-webkit-scrollbar { display: none; }
    .filter-btn { height: 52px; width: auto; cursor: pointer; transition: all 0.25s cubic-bezier(0.2, 0.8, 0.2, 1); opacity: 0.7; filter: grayscale(100%); border-radius: 16px; flex-shrink: 0; }
    .filter-btn:hover, .filter-btn.active { opacity: 1; transform: scale(1.1); filter: grayscale(0%); box-shadow: 0 8px 20px rgba(0,0,0,0.15); }

    /* --- SERVICE LIST (ANIMATED) --- */
    #service-list-container { display: flex; flex-direction: column; gap: 12px; }
    
    .svc-item {
        background: var(--ios-card); border-radius: var(--radius-m); padding: 18px;
        cursor: pointer; box-shadow: var(--shadow-sm); transition: transform 0.2s, box-shadow 0.2s;
        border: 1px solid rgba(0,0,0,0.02); animation: popIn 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
    }
    .svc-item:hover { box-shadow: 0 8px 25px rgba(0,0,0,0.08); transform: translateY(-2px); }
    ...
    .svc-header { display: flex; justify-content: space-between; align-items: flex-start; gap: 12px; margin-bottom: 12px; }
    .svc-title { font-size: 15px; font-weight: 600; color: var(--ios-text); line-height: 1.4; flex: 1; }
    .svc-price { background: rgba(99, 102, 241, 0.1); color: var(--lx-primary); padding: 6px 12px; border-radius: 12px; font-size: 14px; font-weight: 800; white-space: nowrap; }
    .svc-tags { display: flex; gap: 6px; flex-wrap: wrap; margin-top: 8px; }
    .tag { font-size: 10px; font-weight: 600; padding: 4px 8px; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px; letter-spacing: 0.02em; text-transform: uppercase; }
    ...
    /* --- 🚀 MODAL REDESIGN --- */
    .modal-overlay { display: none; position: fixed; inset: 0; background: rgba(0, 0, 0, 0.4); z-index: 2000; justify-content: center; align-items: center; opacity: 0; transition: opacity 0.3s; backdrop-filter: blur(5px); }
    .modal-overlay.active { display: flex; opacity: 1; }

    .modal-content { background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(25px) saturate(180%); width: 92%; max-width: 420px; border-radius: 30px; box-shadow: var(--shadow-lg); border: 1px solid rgba(255, 255, 255, 0.6); display: flex; flex-direction: column; max-height: 92vh; transform: scale(0.9); opacity: 0; transition: transform 0.4s cubic-bezier(0.19, 1, 0.22, 1), opacity 0.3s; overflow: hidden; }
    .modal-overlay.active .modal-content { transform: scale(1); opacity: 1; }

    .modal-header { padding: 16px 20px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(0, 0, 0, 0.05); background: rgba(255, 255, 255, 0.5); }
    .modal-header h3 { margin: 0; font-size: 17px; font-weight: 700; }
    .modal-close { background: rgba(118, 118, 128, 0.12); border: none; width: 30px; height: 30px; border-radius: 50%; color: var(--ios-text-secondary); cursor: pointer; display: flex; align-items: center; justify-content: center; }

    .modal-body { padding: 20px; overflow-y: auto; -webkit-overflow-scrolling: touch; }

    .eta-box-lavish { background: rgba(99, 102, 241, 0.08); border-radius: 20px; padding: 15px; margin-bottom: 20px; text-align: center; color: var(--ios-text); display: none; border: 1px solid rgba(99, 102, 241, 0.2); box-shadow: 0 4px 12px rgba(99, 102, 241, 0.1); }
    .eta-label { font-size: 11px; text-transform: uppercase; letter-spacing: 1px; font-weight: 700; color: var(--ios-text-secondary); }
    .eta-time { font-size: 20px; font-weight: 800; color: var(--lx-primary); margin-top: 4px; display: block; }
    
    .input-group-ios { background: white; border-radius: 18px; padding: 4px 16px; margin-bottom: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.03); }
    ...
    .form-input-ios { flex: 1; border: none; background: transparent; font-size: 15px; text-align: right; font-weight: 600; color: var(--lx-primary); }
    
    .link-wrapper { position: relative; margin-bottom: 16px; }
    .link-input-box { width: 100%; padding: 16px 16px 16px 44px; background: #FFFFFF; border: none; border-radius: 18px; font-size: 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.03); }
    .link-icon { position: absolute; left: 16px; top: 16px; color: var(--ios-text-secondary); }
    .paste-btn-ios { position: absolute; right: 8px; top: 8px; background: rgba(99, 102, 241, 0.1); color: var(--lx-primary); font-size: 11px; font-weight: 800; padding: 6px 10px; border-radius: 10px; border: none; cursor: pointer; }

    .desc-box-scroll { background: rgba(255, 255, 255, 0.5); border-radius: 18px; margin-bottom: 16px; padding: 15px; height: 100px; overflow-y: auto; position: relative; border: 1px solid rgba(0,0,0,0.05); font-size: 13px; color: var(--ios-text-secondary); line-height: 1.5; }
    .desc-box-scroll p { margin: 0; }

    .modal-footer-island { margin: 0 15px 15px 15px; padding: 18px; background: rgba(255, 255, 255, 0.95); border-radius: 24px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); display: flex; flex-direction: column; gap: 12px; border: 1px solid rgba(0,0,0,0.05); }
    ...
    .btn-submit-ios { width: 100%; padding: 16px; background: linear-gradient(135deg, var(--lx-primary) 0%, #4f46e5 100%); color: white; border: none; border-radius: 18px; font-size: 16px; font-weight: 600; box-shadow: 0 8px 20px rgba(99, 102, 241, 0.25); cursor: pointer; position: relative; overflow: hidden; }
    .btn-submit-ios:active { transform: scale(0.97); }

    /* --- PREMIUM FOOTER --- */
    .lavish-footer { text-align: center; padding: 25px; margin-top: 30px; background: rgba(255,255,255,0.4); backdrop-filter: blur(10px); border-radius: 24px 24px 0 0; animation: slideUpFooter 0.8s ease-out; }
    @keyframes slideUpFooter { from { transform: translateY(50px); opacity:0; } to { transform: translateY(0); opacity:1; } }
    .footer-brand { font-weight: 900; color: var(--ios-text); font-size: 18px; margin-bottom: 5px; display:block; letter-spacing: -0.5px; }
    .footer-text { font-size: 12px; color: var(--ios-text-secondary); font-weight: 600; }

    .ios-success-overlay { position: fixed; top:0; left:0; width:100%; height:100%; background: rgba(242, 242, 247, 0.6); backdrop-filter: blur(20px) saturate(180%); z-index: 3000; display:none; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.4s ease; }
    .ios-success-overlay.active { display:flex; opacity:1; }
    .ios-success-card { background: #FFFFFF; width: 320px; border-radius: 32px; padding: 40px 24px; text-align: center; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.15); transform: scale(0.8); transition: transform 0.5s cubic-bezier(0.34, 1.56, 0.64, 1); position: relative; overflow: hidden; }
    .ios-success-overlay.active .ios-success-card { transform: scale(1); }

    .success-icon-container { width: 80px; height: 80px; margin: 0 auto 24px auto; border-radius: 50%; background: var(--lx-primary); display: flex; align-items: center; justify-content: center; box-shadow: 0 10px 20px rgba(99, 102, 241, 0.3); }
    ...
    .btn-check-order { background: rgba(99, 102, 241, 0.1); color: var(--lx-primary); border: none; padding: 12px 24px; border-radius: 20px; font-size: 15px; font-weight: 600; cursor: pointer; width: 100%; transition: background 0.2s; }
    .btn-check-order:hover { background: rgba(99, 102, 241, 0.2); }

    .redirect-bar { height: 4px; width: 100%; background: #F2F2F7; margin-top: 24px; border-radius: 2px; overflow: hidden; }
    .redirect-progress { height: 100%; width: 0%; background: var(--lx-primary); border-radius: 2px; }
    .ios-success-overlay.active .redirect-progress { animation: fillProgress 5s linear forwards; }
    @keyframes fillProgress { to { width: 100%; } }

    /* Mobile */
    @media (max-width: 480px) {
        .modal-overlay { align-items: flex-end; padding: 0; }
        .modal-content { width: 100%; max-width: 100%; border-radius: 32px 32px 0 0; max-height: 90vh; transform: translateY(100%); }
...
<div class="app-wrapper">
    ...
    <!-- 🔥 LIVE ANIMATED SEARCH BOX (WITH PASTE BUTTON) -->
    <div class="search-box-wrapper" id="search-box-wrapper">
        <input type="text" id="global-search" placeholder="">
        <i class="fa fa-search search-icon"></i>
        <button type="button" class="search-paste-btn" onclick="pasteToSearch()">PASTE</button>
    </div>
    ...
    <div class="lavish-footer">
        <span class="footer-brand"><i class="fas fa-crown" style="color: #fbbf24;"></i> LikexFollow.com</span>
        <span class="footer-text">The World's #1 Premium SMM Panel</span>
    </div>
...
<div class="modal-overlay" id="order-modal">
    <div class="modal-content">
        <div class="modal-header">
            <h3 id="modal-service-id" style="font-family: monospace; color: var(--lx-primary);">LXF-0000</h3>
        ...
        <div class="modal-body">
            ...
            <form action="smm_order_action.php" method="POST" id="order-form">
                <!-- IMPORTANT: Backend needs normal numeric ID -->
                ...
                <div class="link-wrapper">
                    ...
                    <button type="button" class="paste-btn-ios" onclick="pasteOrderLink()">PASTE</button>
                </div>
                ...
                <div class="input-group-ios">
                    ...
                    <div class="form-row" style="justify-content:space-between; padding:6px 0;">
                        ...
                        <span id="min-max" style="font-size:11px; color:var(--lx-primary); font-weight:600;"></span>
                    </div>
                </div>
...
<script>
// --- 🔥 TYPEWRITER PLACEHOLDER ANIMATION FOR SEARCH BOX ---
const placeholders = ["Search any service ID etc...", "Search LXF-0012...", "Search TikTok Followers...", "Search Instagram Likes..."];
let phIndex = 0;
let charIndex = 0;
...
function goBack() {
    ...
    // Clear search box if going back
    document.getElementById('global-search').value = '';
    document.getElementById('search-box-wrapper').classList.remove('active-search');

    if(prevView === 'home') navigateTo('view-home');
    if(prevView === 'subcats') navigateTo('view-subcats');
    if(prevView === 'services') navigateTo('view-services');
}
...
// 🔥 FIXED FILTER BUG FOR 'REFILL' VS 'NO REFILL'
function checkKeywords(service, filterType) {
    let rule = window.filterRules[filterType];
    if (!rule) return false;
    
    // Remove empty spaces to prevent buggy matching
    let keys = rule.split(',').map(k => k.trim().toLowerCase()).filter(k => k.length > 0);
    let sName = service.name.toLowerCase();
    let sCat = service.cat.toLowerCase();
    
    // STRICT CHECK: If filtering for 'refill', strictly ignore 'no refill' services
    if(filterType === 'refill') {
        if(sName.includes('no refill') || sName.includes('no-refill') || sCat.includes('no refill') || sCat.includes('no-refill')) {
            return false;
        }
    }
...
// 🔥 CLICK TO COPY FUNCTION (WITH COPY ICON)
function copyServiceId(event, element, text) {
...
function renderServices() {
    ...
    // 🔥 FIXED REFILL LOGIC
        ...
        list = list.filter(s => {
            let nameCat = (s.name + " " + s.cat).toLowerCase();
            if(nameCat.includes('no refill') || nameCat.includes('no-refill')) return false; // Strict Block
            return s.refill === true || checkKeywords(s, 'refill');
        });
    }
    else if (currentFilterType === 'no_refill') {
        list = list.filter(s => {
            let nameCat = (s.name + " " + s.cat).toLowerCase();
            if(nameCat.includes('no refill') || nameCat.includes('no-refill')) return true; // Strict Allow
            return s.refill === false || checkKeywords(s, 'no_refill');
        });
    }
    ...
    if(list.length === 0) noMsg.style.display = 'block';
        ...
        list.forEach((s, index) => {
            ...
            // 🔥 NEW COPY BADGE WITH COPY ICON
            item.innerHTML = `
                <div class="lxf-badge" title="Click to copy ID" onclick="copyServiceId(event, this, '${s.lxf_id}')">
                    <i class="fas fa-copy"></i> ${s.lxf_id}
                </div>
...
function openModal(id) {
    ...
    document.getElementById('modal-service-id').innerText = s.lxf_id; 
...
function submitOrder() { document.getElementById('order-form').requestSubmit(); }

// Order form link paste button
async function pasteOrderLink() {
    try {
...
// 🔥 MAIN SEARCH BAR PASTE FUNCTION
async function pasteToSearch() {
    try {
        const text = await navigator.clipboard.readText();
        const searchInput = document.getElementById('global-search');
        searchInput.value = text;
        // Trigger input event to fire the search logic automatically
        searchInput.dispatchEvent(new Event('input', { bubbles: true }));
    } catch(err) { 
        Swal.fire({ toast:true, position:'top', icon:'error', title:'Clipboard access denied', showConfirmButton:false, timer:1500 });
    }
}
...
document.getElementById('order-form').addEventListener('submit', function(e) {
    ...
    .then(res => {
        ...
        fire(0.25, { spread: 26, startVelocity: 55, colors:['#6366f1','#818cf8'] });
        fire(0.2, { spread: 60 });
        fire(0.35, { spread: 100, decay: 0.91, scalar: 0.8 });
        fire(0.1, { spread: 120, startVelocity: 25, decay: 0.92, scalar: 1.2 });
        fire(0.1, { spread: 120, startVelocity: 45 });

        setTimeout(() => { window.location.href = 'https://likexfollow.com/user/smm_history.php'; }, 5000);
    });
});

// 🔥 DYNAMIC SEARCH LOGIC (LXF ID + Name)
document.getElementById('global-search').addEventListener('input', function(e){
    let q = e.target.value.trim().toLowerCase();
    let wrapper = document.getElementById('search-box-wrapper');

    if(q.length > 0) {
        wrapper.classList.add('active-search');
    } else {
        wrapper.classList.remove('active-search');
    }

    if(q.length >= 2) {
        if(!historyStack.includes('search_results')) {
            historyStack.push('search_results');
        }
        
        navigateTo('view-services');
        document.getElementById('svc-list-title').innerText = "Live Results";
        
        document.getElementById('filter-container').style.display = 'none';
        document.getElementById('svc-app-icon').style.display = 'none';

        // Filter Logic (Matches ID or Name)
        activeServiceList = Object.values(window.svcData).filter(s => {
            let sId = s.lxf_id.toLowerCase();
            let sName = s.name.toLowerCase();
            let sCat = s.cat.toLowerCase();
            
            // Clean the query if user types "lxf-" or just numbers
            let searchClean = q.replace('lxf-', '');
            
            return sId.includes(searchClean) || sName.includes(q) || sCat.includes(q);
        });
